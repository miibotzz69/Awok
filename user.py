...
@client.on(events.NewMessage(pattern=r"^/artinama\s+(.+)", incoming=True))
@safe_run
async def arti_nama_handler(event):
    if not check_access(event.sender_id):
        return await event.reply("🚫 TA BOLEH ASU (khusus owner/admin)")
    ...
    if not nama:
        return await event.reply("❌ Masukin nama setelah /artinama")

    await event.reply("🔍 Mii lagi cari arti nama *{}*...".format(nama))

    api_url = f"https://api.siputzx.my.id/api/primbon/artinama?nama={quote_plus(nama)}"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url) as resp:
                ...
                if not data.get("status") or "result" not in data:
                    return await event.reply("❌ Data tidak valid dari API")

                arti = data["result"].get("arti", "Ga ketemu bre.")

                await event.reply(f"📛 **Arti nama `{nama}`**:\n\n{arti}")

    except Exception as e:
        await event.reply(f"❌ Error: {str(e)}")

@client.on(events.NewMessage(pattern=r'^/ssweb\s+(.+)', incoming=True))
@safe_run
async def screenshot_web_handler(event):
    if not check_access(event.sender_id):
        return await event.reply("🚫 TA BOLEH ASU (khusus owner/admin)")
    ...
    if not url.startswith("http"):
        return await event.reply("❌ Masukin URL yang valid bro, contoh:\n`/ssweb https://google.com`")

    await event.reply("🌐 Mii lagi screenshot web...")

    # Atur tema dan device sesuai kebutuhan
    theme = "light"
    device = "desktop"
    api_url = f"https://api.siputzx.my.id/api/tools/ssweb?url={quote_plus(url)}&theme={theme}&device={device}"
    ...
    try:
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(api_url) as resp:
                ...
                image_data = await resp.read()
                if not image_data:
                    return await event.reply("❌ Screenshot kosong bro.")

        # Simpan PNG ke folder 'foto/ss.png'
        os.makedirs("foto", exist_ok=True)
        file_path = "foto/ss.png"
        with open(file_path, "wb") as f:
            f.write(image_data)
...
@client.on(events.NewMessage(pattern=r"^/tts\s+(.+)", incoming=True))
@safe_run
async def tts_handler(event):
    ...
    if not text:
        return await event.reply("❌ Masukin teks setelah /tts")

    await event.reply("🔊 Mii lagi bikin suaranya...")

    # Parameter voice, rate, pitch, volume bisa diubah kalau mau
    voice = "jv-ID-DimasNeural"
    rate = "0%"
    pitch = "0Hz"
    volume = "0%"

    api_url = f"https://api.siputzx.my.id/api/tools/tts?text={quote_plus(text)}&voice={voice}&rate={rate}&pitch={pitch}&volume={volume}"
    ...
    try:
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(api_url) as resp:
                ...
                if not audio_data:
                    return await event.reply("❌ Gagal dapetin suara.")

        # Simpen MP3 ke folder
        os.makedirs("foto", exist_ok=True)
        audio_path = "foto/luck.mp3"
        with open(audio_path, "wb") as f:
            f.write(audio_data)

        # Kirim MP3 ke Telegram
        await client.send_file(
            event.chat_id,
            audio_path,
            caption=f"🎧 Suara dari: `{text}`",
            voice_note=False,
            reply_to=event.message.id
        )
