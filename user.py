...
@client.on(events.NewMessage(pattern=r"^/artinama\s+(.+)"))
@safe_run
async def arti_nama_handler(event):
    ...
    await event.reply(f"🔍 Mii lagi cari arti nama *{nama}*...")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url) as resp:
                ...
                arti = data["data"].get("arti", "❌ Arti gak ditemukan.")
                await event.reply(f"📛 **Arti nama `{nama}`**:\n\n{arti}")

    except Exception as e:
        await event.reply(f"❌ Error: {str(e)}")

@client.on(events.NewMessage(pattern=r'^/ssweb\s+(.+)'))
@safe_run
...
@client.on(events.NewMessage(pattern=r"^/tts\s+(.+)"))
@safe_run
